...
test("Icon List Snippet", async () => {
    ...
    expect(":iframe .s_icon_list").toHaveStyle("--icon-list-color: #FF0000");
    ...
    expect(":iframe .s_icon_list").toHaveStyle(
        "--icon-list-bg-color: linear-gradient(135deg,rgb(255,204,51) 0%,rgb(226,51,255) 100%)"
    );

    await contains(".options-container button[data-action-id='replaceListIcon']").click();
    await contains(".modal-dialog .fa-remove").click();
    expect(":iframe .s_icon_list").toHaveStyle('--icon-list-content: "\\f00d"');
});
