...
from odoo.addons.populate.generators import NO_VALUE, Boolean, Float, Integer, Monetary
...
class TestMonetaryGenerator(TransactionCase):
    ...
    def test_monetary_generator_basic(self):
        generator = Monetary(field=self.monetary_field, env=self.env, start=10.0, end=1000.0, null_frac=0)

        values = [generator.send({'currency_id': self.usd.id}) for _ in range(50)]

        self.assertTrue(all(isinstance(val, float) for val in values))
        self.assertTrue(all(10.0 <= val <= 1000.0 for val in values))

    def test_monetary_generator_rounds_to_currency_decimal_places(self):
        generator = Monetary(field=self.monetary_field, env=self.env, start=1.0, end=9999.99, null_frac=0)

        for currency in (self.usd, self.eur):
            values = [generator.send({'currency_id': currency.id}) for _ in range(50)]
            for value in values:
    ...
    def test_monetary_generator_different_decimal_places(self):
        ...
        if jpy.decimal_places == 0:
            generator = Monetary(field=self.monetary_field, env=self.env, start=100.0, end=10000.0, null_frac=0)
            values = [generator.send({'currency_id': jpy.id}) for _ in range(20)]
            for value in values:
                self.assertEqual(value, round(value, 0))

    def test_monetary_generator_depends_on_currency_field(self):
        generator = Monetary(field=self.monetary_field, env=self.env, null_frac=0)
    ...
    def test_monetary_generator_returns_no_value_when_currency_missing(self):
        generator = Monetary(field=self.monetary_field, env=self.env, null_frac=0)
    ...
    def test_monetary_generator_without_currency_in_known_vals(self):
        generator = Monetary(field=self.monetary_field, env=self.env, start=1.0, end=500.0, null_frac=0)

        # When currency_id is present but falsy (0 / False), no rounding by currency is applied
        value = generator.send({'currency_id': False})
        self.assertIsInstance(value, float)
        self.assertTrue(1.0 <= value <= 500.0)

    def test_monetary_generator_null_frac(self):
        generator = Monetary(field=self.monetary_field, env=self.env, null_frac=1.0)

        values = [generator.send({'currency_id': self.usd.id}) for _ in range(20)]
        self.assertTrue(all(v is False for v in values))

    def test_monetary_generator_custom_range(self):
        generator = Monetary(field=self.monetary_field, env=self.env, start=50.0, end=100.0, null_frac=0)
    ...
    def test_monetary_generator_values_vary(self):
        generator = Monetary(field=self.monetary_field, env=self.env, start=1.0, end=1000000.0, null_frac=0)
    ...
    def test_monetary_get_kwargs(self):
        attrs = {
