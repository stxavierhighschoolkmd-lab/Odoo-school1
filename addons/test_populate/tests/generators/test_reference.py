...
from odoo.addons.populate.generators import NO_VALUE, ReferenceOne, ReferenceRaw
...
class TestReferenceOne(TransactionCase):
    ...
    def test_reference_one_generator_basic(self):
        ...
        generator = ReferenceOne(field=self.res_id_field, env=self.env, null_frac=0)

        values = []
        for _ in range(10):
            value = generator.send({'res_model': 'test_populate.product'})
            values.append(value)
    ...
    def test_reference_one_generator_different_models(self):
        ...
        product_values = [generator.send({'res_model': 'test_populate.product'}) for _ in range(10)]
        for value in product_values:
            self.assertIn(value, products.ids)

        supplier_values = [generator.send({'res_model': 'test_populate.supplier'}) for _ in range(10)]
...
class TestReferenceRaw(TransactionCase):
    ...
    def test_reference_raw_generator_basic(self):
        ...
        generator = ReferenceRaw(field=self.reference_field, env=self.env, null_frac=0)
    ...
    def test_reference_raw_generator_with_depends_model_only(self):
        ...
        values = [generator.send({'res_model': 'test_populate.product'}) for _ in range(10)]
    ...
    def test_reference_raw_generator_null_frac(self):
        self.env['test_populate.product'].create([
            {'name': 'Product A', 'price': 10.0},
        ])

        generator = ReferenceRaw(field=self.reference_field, env=self.env, null_frac=0.9)
    ...
    def test_reference_raw_generator_invalid_model_in_depends(self):
        ...
        with self.assertRaises(ValueError) as cm:
            generator.send({'res_model': 'invalid.model.name'})

        self.assertIn("not in the allowed models", str(cm.exception))

    def test_reference_raw_generator_get_kwargs(self):
        attrs = {
        ...
        kwargs = ReferenceRaw.get_kwargs(attrs)

        self.assertIn('ref', kwargs)
        self.assertEqual(kwargs['ref'], 'special_products')
        self.assertIn('null_frac', kwargs)
        self.assertEqual(kwargs['null_frac'], 0.2)


class TestReferenceRawSessionBinding(TransactionCase):
