...
class IrAccess(models.Model):
    ...
    def _search_for_read(self, operator, value):
        if operator != 'in':
            return NotImplemented
        return Domain('operation', 'in', [op for op, _ in OPERATION_SELECTION if 'r' in op])

    def _search_for_write(self, operator, value):
        if operator != 'in':
            return NotImplemented
        return Domain('operation', 'in', [op for op, _ in OPERATION_SELECTION if 'w' in op])

    def _search_for_create(self, operator, value):
        if operator != 'in':
            return NotImplemented
        return Domain('operation', 'in', [op for op, _ in OPERATION_SELECTION if 'c' in op])

    def _search_for_unlink(self, operator, value):
        if operator != 'in':
            return NotImplemented
        return Domain('operation', 'in', [op for op, _ in OPERATION_SELECTION if 'd' in op])
        ...
        for access in self:
            if access.active and access.domain:
                try:
                    ...
                    raise ValidationError(_('Invalid domain %(domain)s: %(error)s', domain=access.domain, error=e))
    ...
    def _get_domain_for(self, model_name: str, operation: str, include_inherits=True) -> Domain:
        ...
        accesses = self._get_records_for(model_name, operation)
    ...
    def _get_groups_with_access(self, model_name: str, operation: str) -> models.Model:
        ...
        groups = self.env['res.groups'].sudo().search_fetch(domain, ['display_name'])

        if parent_model_names := self.env[model_name]._inherits:
            # find groups that imply groups having access to all parent models as well
            groups_list = [groups, *(
                self._get_groups_with_access(parent_model_name, operation)
                for parent_model_name in parent_model_names
            )]
            domain = Domain.AND(
                Domain('all_implied_ids', 'in', groups.ids)
                for groups in groups_list
            )
            groups = groups.search_fetch(domain, ['display_name'])
    ...
    def _get_failed_accesses(self, records, operation: str):
        ...
        accesses = self._get_records_for(model._name, operation)
