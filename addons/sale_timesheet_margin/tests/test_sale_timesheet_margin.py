...
class TestSaleTimesheetMargin(TestCommonSaleTimesheet):
    ...
    def test_compute_purchase_price_new_line_after_so_confirm(self):
        ...
        Scenario:
        1. Create SO with product without cost (standard_price=0)
        2. Confirm SO
        3. Add new line with product that has cost (standard_price=30)
        4. Verify the new line's purchase_price is computed from product cost
        """

        product_no_cost, product_with_cost = self.env['product.product'].create([{
            'name': 'Service Product - No Cost',
            'list_price': 50.0,
        ...
        self.assertEqual(initial_line.purchase_price, 0.0)
        sale_order.action_confirm()
