...
export class SelectionPlugin extends Plugin {
    ...
    resources = {
        ...
        // TODO AGE: maybe we should use Infinity here. It might be important to
        // be last. Note: it at the very least must be done after `DomMutation`,
        // but `Selection` is loaded before `DomMutation` due to their
        // dependencies. We could also put a sequence of 0 on `DomMutation`'s
        // handler, but I think this it's more relevant here.
        on_revert_commit_handlers: withSequence(15, (commit, { restoreFocus = true } = {}) => {
    ...
    processCommitData(data) {
        // Make sure the selection after is always present.
        this.currentData.selectionAfter =
            this.currentData.selectionAfter ?? this.serializeEditableSelection();
