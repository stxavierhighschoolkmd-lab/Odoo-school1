...
test("Uploading attachment order, latest upload first", async () => {
    ...
    onRpc("/mail/attachment/upload", () => Promise.withResolvers().promise); // never fulfill the attachment upload promise.
    await start();
    await openFormView("res.partner", serverState.partnerId, {
        arch: `
            <form>
    ...
    await contains(".o-mail-AttachmentList .o-mail-AttachmentContainer:eq(0) [title='Uploading']");
    await contains(
