...
export class VisibilityPlugin extends Plugin {
    ...
    resources = {
        ...
        on_step_added_handlers: ({ step: { type, extraStepInfos }, isPreviewing }) => {
            ...
            if (
                optionsTarget &&
                !(this.checkPredicates("is_element_shown_predicates", optionsTarget) ?? true)
            ) {
                if (type === "original") {
        ...
        is_element_shown_predicates: (el) => {
    ...
    // `on_target_revealed_handlers` won't be called for them on redo. This function
    ...
    showNewElement(el, isClone) {
        ...
        if (shouldShow) {
            this.dependencies.history.applyCustomMutation({
                apply: () => {
                    if (!(this.checkPredicates("is_element_shown_predicates", el) ?? true)) {
