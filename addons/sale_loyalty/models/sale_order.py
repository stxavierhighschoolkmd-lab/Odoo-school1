...
class SaleOrder(models.Model):
    ...
    )  # TODO(loti): `loyalty_history_data` would less vague.
    ...
    def _compute_loyalty_data(self):
        ...
        loyalty_history_data = (
            ...
            ._read_group(
                ...
                ],  # TODO(loti): for clarity, I wouldn't use `self._name`.
                groupby=["order_id"],
        ...
        for order in self:
            ...
            if any(order._get_real_points_for_coupon(coupon) < 0 for coupon in all_coupons):
                raise ValidationError(
                    _("One or more rewards on the sale order are invalid. Please check them.")
                )
    ...
    def _get_real_points_for_coupon(self, coupon):
        ...
        return coupon.currency_id.round(points)  # TODO(loti): currency???
    ...
    def _program_check_compute_points(self, programs):
        ...
        )  # TODO(loti): we should skip reward lines when computing quantities (since they shouldn't
        # count in the min product quantity to reach). Note: rename _get_not_rewarded_order_lines (which isn't clear IMO).
        ...
        for program in programs:
            ...
            if not program.is_nominative:
                ...
                self.partner_id.is_public and not self._allow_nominative_programs()
