...
class MrpWorkcenter(models.Model):
    ...
    def _get_workcenter_load_per_week(self, week_range, date_start, date_stop):
        ...
        count_workorders = self.env['mrp.workorder'].search_count([('workcenter_id', 'in', self.ids)], limit=1)
        if not count_workorders:
            for wc in self:
    ...
    def _prepare_graph_data(self, load_data, week_range):
        ...
        for workcenter in self:
            ...
            wc_data = {'is_sample_data': not count_workorders, 'labels': list(week_range.values())}
            load_bar = []
            excess_bar = []
