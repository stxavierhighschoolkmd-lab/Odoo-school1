...
class TestBinaryGenerator(TransactionCase):
    ...
    def test_binary_default_size(self):
        generator = Binary(field=self.binary_field, env=self.env, null_frac=0)
    ...
    def test_binary_custom_size(self):
        generator = Binary(field=self.binary_field, env=self.env, size=512, null_frac=0)
        value = next(generator)
    ...
    def test_binary_null_frac(self):
        generator = Binary(field=self.binary_field, env=self.env, null_frac=1.0)
    ...
    def test_binary_get_kwargs(self):
        attrs = {
            'generator': 'binary.binary',
            'size': '256',
            'null_frac': '0.1',
        }
        kwargs = Binary.get_kwargs(attrs)
...
class TestImageGenerator(TransactionCase):
    ...
    def test_image_default_dimensions(self):
        generator = Image(field=self.binary_field, env=self.env, null_frac=0)
        value = next(generator)
    ...
    def test_image_custom_dimensions(self):
        generator = Image(field=self.binary_field, env=self.env, width=32, height=16, null_frac=0)
        value = next(generator)
    ...
    def test_image_colors_vary(self):
        generator = Image(field=self.binary_field, env=self.env, null_frac=0)
        images = [next(generator) for _ in range(10)]
    ...
    def test_image_null_frac(self):
        generator = Image(field=self.binary_field, env=self.env, null_frac=1.0)
        values = [next(generator) for _ in range(20)]
    ...
    def test_image_get_kwargs(self):
        attrs = {
        ...
        kwargs = Image.get_kwargs(attrs)

        self.assertEqual(kwargs['width'], 128)
        self.assertEqual(kwargs['height'], 256)
        self.assertAlmostEqual(kwargs['null_frac'], 0.2)
