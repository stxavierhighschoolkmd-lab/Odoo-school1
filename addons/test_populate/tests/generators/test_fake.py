...
class TestFakeGeneratorMethods(TransactionCase):
    ...
    def test_create_generator_class(self):
        ...
        FakeName = Generator.get('fake.name')
...
class TestFakeGenerators(TransactionCase):
    ...
    def test_fake_generator_null_frac(self):
        test_field = self.notes_field
        assert not test_field.required, "No point testing for null_frac, if field is required. (null_frac -> 0)"
        FakeName = Generator.get('fake.words')
        generator = FakeName(field=test_field, env=self.env, null_frac=0.9)
    ...
    def test_fake_generator_no_null_with_zero_frac(self):
        FakeName = Generator.get('fake.name')
        generator = FakeName(field=self.name_field, env=self.env, null_frac=0)
    ...
    def test_fake_generator_unique(self):
        FakeEmail = Generator.get('fake.email')
        generator = FakeEmail(field=self.email_field, env=self.env, unique=True, null_frac=0)

        values = [next(generator) for _ in range(50)]

        self.assertEqual(len(values), len(set(values)))

    def test_fake_generator_get_kwargs_basic(self):
        FakeName = Generator.get('fake.name')
        attrs = {
            'generator': 'fake.name',
            'locale': 'de_DE',
        ...
        kwargs = FakeName.get_kwargs(attrs)

        self.assertEqual(kwargs['locale'], 'de_DE')
        self.assertEqual(kwargs['null_frac'], 0.2)

    def test_fake_generator_get_kwargs_conversion_bool(self):
        FakeName = Generator.get('fake.name')
    ...
    def test_fake_generator_get_kwargs_conversion_int(self):
        FakeName = Generator.get('fake.name')
    ...
    def test_fake_generator_get_kwargs_conversion_float(self):
        FakeName = Generator.get('fake.name')
        attrs = {
            'decimal': '3.14',
            'negative': '-2.5',
    ...
    def test_fake_generator_get_kwargs_conversion_list(self):
        FakeName = Generator.get('fake.name')
        attrs = {
            'elements': '[Mr, Mrs, Ms]',
            'numbers': '(1, 2, 3)',
    ...
    def test_fake_generator_get_kwargs_conversion_none(self):
        FakeName = Generator.get('fake.name')
    ...
    def test_fake_generator_with_method_args(self):
        ...
        try:
            FakeNumerify = Generator.get(f'fake.{method_name}')
            if FakeNumerify:
