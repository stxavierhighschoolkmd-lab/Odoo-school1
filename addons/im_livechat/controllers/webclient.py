...
class WebClient(WebclientController):
    ...
    @mail_data_handler("im_livechat.channel")
    def _mail_data_im_livechat_channel(self, store: Store):
        store.add(request.env["im_livechat.channel"].search([]), ["are_you_inside", "name"])

    @mail_data_handler("/im_livechat/looking_for_help")
    def _mail_data_im_livechat_looking_for_help(self, store: Store):
    ...
    @mail_data_handler("/im_livechat/session/data")
    def _mail_data_im_livechat_session_data(self, store: Store, channel_id):
        if not channel_id:
            return
        channel = request.env["discuss.channel"].search([("id", "=", channel_id)])
        store.add(channel, "_store_livechat_extra_fields")

    @mail_data_handler("/im_livechat/fetch_self_expertise")
    def _mail_data_im_livechat_fetch_self_expertise(self, store: Store):
        store.add(request.env.user, lambda res: res.many("livechat_expertise_ids", ["name"]))

    @mail_data_handler("init_livechat", target=MailDataTarget.ALL)
    def _mail_data_init_livechat(self, store, livechat_channel_id=None):
