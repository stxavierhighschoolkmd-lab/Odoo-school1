...
class TestVNEDIStock(TransactionCase):
    ...
    def setUpClass(cls):
        ...
        cls.company_vn.l10n_vn_edi_default_sinvoice_symbol_id = cls.symbol
    ...
    def test_show_send_button_transit(self):
        ...
        picking.l10n_vn_edi_state = 'sent'
    ...
    def test_symbol_resolution_warehouse_then_company(self):
        ...
        # Clearing warehouse symbol falls back to company default
        self.wh1.l10n_vn_edi_sinvoice_symbol_id = False
        picking.invalidate_recordset(['l10n_vn_edi_symbol_id'])
    ...
    def test_auto_population_of_fields(self):
        """Economic contract no, export/import warehouse names are auto-populated."""
        picking = self._make_inter_warehouse_picking(state='assigned')

        self.assertEqual(picking.l10n_vn_edi_economic_contract_no, picking.name)
        self.assertEqual(picking.l10n_vn_edi_export_at, self.wh1.name)
        self.assertEqual(picking.l10n_vn_edi_import_at, self.wh2.name)

        # Manual override must not be overwritten by recompute
        picking.l10n_vn_edi_export_at = 'Manual Override'
        picking.invalidate_recordset(['l10n_vn_edi_export_at'])
        self.assertEqual(picking.l10n_vn_edi_export_at, 'Manual Override')
    ...
    def test_json_general_seller_buyer_info(self):
        ...
        # Required metadata key must be present
        meta_keys = [m['keyTag'] for m in data['metadata']]
        self.assertIn('economicContractNo', meta_keys)
    ...
    def test_send_transfer_note_success(self):
        ...
        self.assertEqual(picking.l10n_vn_edi_state, 'sent')
        self.assertEqual(picking.l10n_vn_edi_invoice_number, 'K24NTU01')
        self.assertEqual(picking.l10n_vn_edi_reservation_code, 'SEC456')
        self.assertTrue(picking.l10n_vn_edi_sinvoice_file)
        self.assertTrue(picking.l10n_vn_edi_sinvoice_xml_file)
        self.assertTrue(picking.l10n_vn_edi_sinvoice_pdf_file)
    ...
    def test_send_transfer_note_api_error(self):
        ...
        with patch('odoo.addons.l10n_vn_edi_viettel.models.res_company.ResCompany._l10n_vn_edi_get_access_token', return_value=(None, 'Auth failed')):
            ...
            with self.assertRaisesRegex(UserError, 'Auth failed'):
                wizard.action_send()
        self.assertFalse(picking.l10n_vn_edi_state)
