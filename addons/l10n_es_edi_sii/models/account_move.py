...
class AccountMove(models.Model):
    ...
    def _compute_l10n_es_edi_sii_data(self):
        for move in self:
            if not move.l10n_es_edi_is_required or move.state != 'posted':
                move.l10n_es_edi_sii_state = False
                move.l10n_es_edi_csv = False
                move.l10n_es_edi_sii_error = False
                continue

            docs = move.l10n_es_edi_sii_document_ids.sorted('create_date', reverse=True)
            accepted_docs = docs.filtered(lambda d: d.state in ('accepted', 'accepted_with_errors'))

            move.l10n_es_edi_csv = accepted_docs[0].csv if accepted_docs else False
    ...
    def _compute_l10n_es_edi_is_required(self):
        for move in self:
            ...
            if move.is_purchase_document():
                has_tax = any(t.l10n_es_type and t.l10n_es_type != 'ignore' for t in move.invoice_line_ids.tax_ids)
    ...
    def button_cancel(self):
        for move in self:
            if move.l10n_es_edi_is_required and move.l10n_es_edi_sii_state == 'sent':
                res = move._send_l10n_es_sii_document(cancel=True)
                if not res or move.l10n_es_edi_sii_state != 'cancelled':
                    error_details = move.l10n_es_edi_sii_error or self.env._("No Specific error message provided by the SII")
                    raise UserError(self.env._(
                        "We could not cancel this invoice because the SII rejected the cancellation request.\n\n"
                        "Reason: %s", error_details
                    ))
    ...
    def _l10n_es_edi_cancel_invoice(self):
        self.ensure_one()
        self._send_l10n_es_sii_document(cancel=True)

        if self.l10n_es_edi_sii_state == 'cancelled':
            self.message_post(body=self.env._("Invoice successfully cancelled at SII (Spain Tax Agency)."))
    ...
    def _send_l10n_es_sii_document(self, cancel=False):
        ...
        if self.l10n_es_edi_csv and not cancel:
        ...
        if document.state in ('accepted', 'accepted_with_errors', 'cancelled'):
            self.l10n_es_edi_sii_document_ids.filtered(lambda d: d.state == 'rejected').sudo().unlink()
    ...
    def _l10n_es_sii_check_move_configuration(self):
        ...
        for line in lines:
            ...
            duplicate_types = {t for t in l10n_types if l10n_types.count(t) > 1}
            if duplicate_types:
                errors.append(
                    self.env._(
            ...
            main_tax_count = (
                l10n_types.count('sujeto')
                + l10n_types.count('no_sujeto')
                + l10n_types.count('no_sujeto_loc')
            )
            if main_tax_count > 1:
                errors.append(
        ...
        if self.is_outbound() and is_foreign and total_taxes:
            has_no_scope = not any(t.tax_scope for t in total_taxes)
            if has_no_scope:
                errors.append(
                    self.env._(
                        "Foreign customer detected. Please configure tax scope on taxes: %s",
    ...
    def _l10n_es_edi_get_invoices_info(self):
        ...
        if self.is_sale_document():
            if self.company_id.vat.startswith('ES'):
        ...
        if self.is_sale_document():
            if not com_partner._l10n_es_is_foreign() or is_simplified:
                ...
                invoice_node['ImporteTotal'] = json_float_round(sign * total_amount, 2)
            else:
                ...
                invoice_node['ImporteTotal'] = json_float_round(sign * (service_total + consu_total), 2)
        else:
            ...
            if self._l10n_es_is_dua() or has_ignore_tax:
                ...
                invoice_node['ImporteTotal'] = json_float_round(sign * (isp_total + other_total), 2)
            else:
                ...
                invoice_node['ImporteTotal'] = json_float_round(
            ...
            invoice_node['CuotaDeducible'] = json_float_round(sign * total_deductible, 2)
    ...
    def _l10n_es_edi_get_invoices_tax_details_info(self, filter_invl_to_apply=None):
        ...
        for tax_values in tax_details['tax_details'].values():
            ...
            if self.is_sale_document():
                match tax_values['l10n_es_type']:
                    case 'sujeto' | 'sujeto_isp':
                        ...
                        tax_info = {
                            ...
                            'BaseImponible': json_float_round(base_amount, 2),
                            'CuotaRepercutida': json_float_round(math.copysign(tax_values['tax_amount'], base_amount), 2)
                        }

                        if recargo:
                            tax_info['CuotaRecargoEquivalencia'] = json_float_round(sign * recargo['tax_amount'], 2)
                    ...
                    case 'exento':
                        ...
                        exenta_dict['DetalleExenta'].append({
                            'BaseImponible': json_float_round(sign * tax_values['base_amount'], 2),
                ...
                if tax_values['l10n_es_type'] not in ['retencion', 'recargo', 'ignore']:
                    base_amount = sign * tax_values['base_amount']
                    tax_info = {'BaseImponible': json_float_round(base_amount, 2)}

                    if tax_values['l10n_es_type'] == 'sujeto_agricultura':
                        tax_info.update({
                            'PorcentCompensacionREAGYP': tax_values['applied_tax_amount'],
                            'ImporteCompensacionREAGYP': json_float_round(math.copysign(tax_values['tax_amount'], base_amount), 2)
                        })
                    elif tax_values['applied_tax_amount'] > 0.0:
                        tax_info.update({
                            'TipoImpositivo': tax_values['applied_tax_amount'],
                            'CuotaSoportada': json_float_round(math.copysign(tax_values['tax_amount'], base_amount), 2)
                        })
        ...
        if not self.company_id.currency_id.is_zero(base_amount_not_subject) and self.is_sale_document():
            tax_details_info.setdefault('NoSujeta', {})['ImportePorArticulos7_14_Otros'] = json_float_round(sign * base_amount_not_subject, 2)

        if not self.company_id.currency_id.is_zero(base_amount_not_subject_loc) and self.is_sale_document():
            tax_details_info.setdefault('NoSujeta', {})['ImporteTAIReglasLocalizacion'] = json_float_round(sign * base_amount_not_subject_loc, 2)
