    ...
    def _compute_value(self):
        ...
        for company in self.env.companies:
            ...
            for cost_method, product_ids in product_ids_grouped_by_cost_method.items():
                products = products.env['product.product'].browse(product_ids).with_context(warehouse_id=False, available_quantities=available_quantities_whole_company)
        ...
        for product in self:
            ...
            product.avg_cost = product.total_value / available_quantities[product.id] if available_quantities.get(product.id) else std_price_by_company_id[self.env.company.id].get(product.id, product.standard_price)
