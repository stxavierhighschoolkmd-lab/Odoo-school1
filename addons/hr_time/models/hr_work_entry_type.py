...
class HrWorkEntryType(models.Model):
    ...
    leave_notif_subtype_id = fields.Many2one('mail.message.subtype', string='Time Off Notification Subtype', default=lambda self: self.env.ref('hr_time.mt_leave', raise_if_not_found=False))
    allocation_notif_subtype_id = fields.Many2one('mail.message.subtype', string='Allocation Notification Subtype', default=lambda self: self.env.ref('hr_time.mt_leave_allocation', raise_if_not_found=False))
    ...
    def _search_valid(self, operator, value):
        ...
        work_entry_types = self.env['hr.time.allocation'].search([
    ...
    def get_work_entry_types_with_valid_allocations(self, date_from, date_to, employee_id):
        allocation_by_work_entry_type = dict(self.env['hr.time.allocation']._read_group(
            domain=Domain([
        ...
        for work_entry_type in self:
            if work_entry_type.requires_allocation:
                allocations = allocation_by_work_entry_type.get(work_entry_type, self.env['hr.time.allocation'])
    ...
    def check_allocation_requirement_edit_validity(self):
        if not self.env.context.get('install_mode') and self.env['hr.time'].search_count([('work_entry_type_id', 'in', self.ids)], limit=1):
    ...
    def _compute_allocation_count(self):
        ...
        grouped_res = self.env['hr.time.allocation']._read_group(
            domain,
            ['work_entry_type_id'],
            ['__count'],
        )
    ...
    def _compute_group_days_leave(self):
        ...
        grouped_res = self.env['hr.time']._read_group(
            domain,
            ['work_entry_type_id'],
            ['__count'],
        )
    ...
    def _leaves_count_by_work_entry_type_id(self):
        ...
        leaves_count = self.env['hr.time']._read_group(
            leave_domain,
            ['work_entry_type_id'],
            ['__count'],
        )
    ...
    def _allocations_count_by_work_entry_type_id(self):
        ...
        allocations_count = self.env['hr.time.allocation']._read_group(
            allocation_domain,
            ['work_entry_type_id'],
            ['__count'],
        )
    ...
    def action_see_days_allocated(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("hr_time.hr_time_allocation_action_all")
        action['domain'] = [
            ('work_entry_type_id', 'in', self.ids),
        ]
        action['context'] = {
    ...
    def has_accrual_allocation(self):
        ...
        if not employee:
            return False
        return bool(self.env['hr.time.allocation'].search_count([
            ('employee_id', '=', employee.id),
            ('state', '=', 'validate'),
    ...
    def get_allocation_data(self, employees, target_date=None):
        ...
        for employee in employees:
            for work_entry_type in self:
                ...
                allocations_now = self.env['hr.time.allocation']
                allocations_date = self.env['hr.time.allocation']
                allocations_with_remaining_leaves = self.env['hr.time.allocation']
    ...
    def _get_carried_over_days_expiration_data(self, allocations, target_date):
        fake_allocations = self.env['hr.time.allocation']
        for allocation in allocations:
            fake_allocations |= self.env['hr.time.allocation'].new(origin=allocation)
