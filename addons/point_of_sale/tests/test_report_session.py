...
class TestReportSession(TestPoSCommon):
    ...
    def test_report_sale_details_total_with_cash_rounding(self):
        """Test that the sale details report total includes cash rounding."""
        ...
        payments_total = sum(p['total'] for p in report['payments'] if 'total' in p)
        self.assertAlmostEqual(
            report['currency']['total_paid'], payments_total, places=2,
            msg="Sale details total should match the sum of payment amounts (including cash rounding)"
        )
