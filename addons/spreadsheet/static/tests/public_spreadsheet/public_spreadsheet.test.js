...
describe("sheet_id URL synchronization", () => {
    ...
    test("activates sheet from URL on initialization", async () => {
        ...
        const { comp } = await mountPublicSpreadsheet("dashboardDataUrl", "spreadsheet");
        expect(comp.model.getters.getActiveSheetId()).toBe("sheet2");
    });
    ...
    test("falls back to the first sheet and syncs the URL when sheet_id is invalid", async () => {
        ...
        expect(comp.model.getters.getActiveSheetId()).toBe("sheet1");
    });
    ...
    test("syncs the URL when the active sheet changes", async () => {
        const { comp } = await mountPublicSpreadsheet("dashboardDataUrl", "spreadsheet");
        comp.model.dispatch("ACTIVATE_SHEET", {
            sheetIdFrom: "sheet1",
            sheetIdTo: "sheet2",
        });
