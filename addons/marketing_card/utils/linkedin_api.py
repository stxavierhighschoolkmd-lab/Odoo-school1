...
class LinkedInAPI:
    ...
    def fetch_access_token(self, linkedin_authorization_code, redirect_uri):
        ...
        self.set_access_token(response.get('access_token'))
    ...
    def ugc_upload_image(self, image_bytes, owner_id):
        ...
        if len(image_bytes) > 100_000_000:
            raise UserError(_('The image is too large: %(image_byte_length)s', len(image_bytes)))
        ...
        upload_res = requests.put(upload_url, headers=headers, data=image_bytes, timeout=60)
        if not upload_res.ok:
            raise UserError(_('Error when uploading an image'))
        return image_urn

    def ugc_post_image(self, author_id, image_urn, image_title, text="", image_description=""):
        """Post 1 image and some text on the author's public feed.
        
        :param author_id: urn id of the author, expected to be a person.
        :param image_urn: full urn of the uploaded image resource.
