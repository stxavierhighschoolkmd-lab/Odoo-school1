...
(`generator`, `eval`, `null_frac`, `domain`, `ref`, `virtual`, etc.).
...
```xml
<field name="project_id" generator="relation.one" null_frac="0"/>
...
<model name="project.project" count="10" id="my_projects">
    <field name="name" generator="fake.bs" null_frac="0"/>
</model>
<model name="project.task" count="100" id="my_tasks">
    <field name="project_id" generator="relation.one" ref="my_projects" null_frac="0"/>
</model>
...
| `NotNullViolation`   | A required column received `NULL`                | Add `null_frac="0"` on the offending field                        |
