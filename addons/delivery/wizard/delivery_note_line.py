...
class DeliveryNoteLine(models.TransientModel):
    ...
    note_id = fields.Many2one(string="Delivery Note Reference", comodel_name="delivery.note")
    product_image = fields.Image(related="product_id.image_128")
    product_id = fields.Many2one(
        string="Product", comodel_name="product.product", related="sale_order_line_id.product_id"
    )
    product_uom_id = fields.Many2one(
        string="Unit", comodel_name="uom.uom", related="sale_order_line_id.product_uom_id"
    )
    product_uom_qty = fields.Float(string="Quantity")
    quantity_ordered = fields.Float(string="Quantity Ordered")
    sale_order_line_id = fields.Many2one(
        string="Source Sale Order Line", comodel_name="sale.order.line"
    )
