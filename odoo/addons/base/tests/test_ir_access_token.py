...
class TestIrAccessToken(TransactionCase):
    ...
    def test_grant_access_to_record_with_manual_token(self):
        ...
        record = self.model.with_user(self.user_1).get_record_from_access_token('access-record', '123456789', record=self.record)
        self.assertEqual(record, self.record, 'The record must be found')
    ...
    def test_retrieve_record_from_concurrent_token(self):
        ...
        record = self.model.with_user(self.admin).get_record_from_access_token('access-record', token, record=self.record)
        self.assertEqual(record, self.record)
