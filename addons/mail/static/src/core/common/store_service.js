...
export class Store extends BaseStore {
    ...
    getMentionsFromText(
        ...
        let segments = [body];
        // If the body contains markup, extract the text content of each anchor tag to improve mention detection.
        if (isMarkup(body)) {
            const bodyParsed = new DOMParser().parseFromString(body, "text/html");
            segments = Array.from(bodyParsed.querySelectorAll("a"), (anchor) => anchor.innerText);
        }
        const validMentions = {};
