...
class ResCompany(models.Model):
    ...
    def _compute_l10n_sa_is_vat_group_member(self):
        for company in self:
            ...
            vat = re.sub(r'\D', '', company.vat or '')
            company.l10n_sa_is_vat_group_member = (
        ...
        for company in self:
            ...
            if company.l10n_sa_additional_identification_scheme != 'TIN':
                raise ValidationError(_(
                    "To comply with ZATCA VAT Group onboarding rules, please set "
                    "Additional Identification Scheme as 'TIN'."
                ))
            tin = re.sub(r'\D', '', company.l10n_sa_additional_identification_number or '')
            if len(tin) != 10:
                raise ValidationError(_(
                    "To comply with ZATCA VAT Group onboarding rules, please set "
                    "Additional Identification Number as the TIN Number(10 digits)."
                ))
