...
contract_costs AS (
    ...
    LEFT JOIN LATERAL (
        SELECT
            ...
            SUM(
                CASE
                    WHEN c.cost_frequency = 'no' AND date_trunc('month', c.date) = date_trunc('month', d)
                    THEN c.amount
                    WHEN c.cost_frequency = 'daily' AND date_trunc('month', c.start_date) <= date_trunc('month', d)
                    ...
                    WHEN c.cost_frequency = 'monthly' AND date_trunc('month', c.start_date) <= date_trunc('month', d) AND date_trunc('month', c.expiration_date) >= date_trunc('month', d)
                    THEN c.cost_generated
                    WHEN c.cost_frequency = 'yearly' AND d BETWEEN c.start_date and c.expiration_date AND date_part('month', c.date) = date_part('month', d)
                    THEN c.cost_generated
                    ELSE 0
                END
            ) AS total_cost
