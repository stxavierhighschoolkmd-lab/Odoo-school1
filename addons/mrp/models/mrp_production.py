...
class MrpProduction(models.Model):
    ...
    def _post_inventory(self, cancel_backorder=False):
        # This handles the case where _post_inventory is called directly
        # (without going through button_mark_done and pre_button_mark_done
        # which also marks moves as picked).
        for production in self:
    ...
    def button_mark_done(self):
        ...
        self.filtered(lambda p: not p.uom_id.is_zero(production.qty_producing)).move_raw_ids.picked = True
    ...
    def pre_button_mark_done(self):
        ...
        for production in self:
            if production._auto_production_checks() and production.uom_id.is_zero(production.qty_producing):
    ...
    def _set_quantities(self):
        ...
        self.move_raw_ids.filtered(lambda move: not move.picked and move.quantity).picked = True

        missing_lot_id_products = ""
        for move in self.move_raw_ids:
            if move.state in ('done', 'cancel') or not move.product_uom_qty:
                continue
            if move.picked and move.has_tracking in ('serial', 'lot') and any(not line.lot_id for line in move.move_line_ids if line.quantity):
