...
class TestPoSSale(TestPointOfSaleHttpCommon):
    ...
    def test_settle_cancelled_order_with_extra_product(self):
        """When settling a cancelled (reset to draft) SO in POS with an extra product,
        the POS picking should include moves for all products, not just the extra one."""
        ...
        self.main_pos_config.open_ui()
        self.start_pos_tour('test_settle_cancelled_order_with_extra_product', login="accountman", watch=False, step_delay=0)
        ...
        self.assertEqual(pos_shipped_products, product_a | product_b | product_c)
