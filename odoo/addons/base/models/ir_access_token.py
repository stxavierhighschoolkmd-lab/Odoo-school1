...
class IrAccessToken(models.Model):
    ...
    def retrieve_from_token(self, scope: str, token: str, *,
        record: BaseModel | None = None) -> Self:
        """ Retrieve access token record from a token and scope. """
        try:
            payload = verify_hash_signed(self.env, scope, token)

        except ValueError:
            assert record, 'A record must be provided for the current token format.'
            domain = [
        ...
        for access_token in self.search_fetch(domain):
...
class Base(models.AbstractModel):
    ...
    def get_record_from_access_token(self, scope: str, token: str, *,
        record: BaseModel | None = None) -> BaseModel:
        ...
        """
        access_token = self.env['ir.access.token'].sudo().retrieve_from_token(scope, token, record=record)
