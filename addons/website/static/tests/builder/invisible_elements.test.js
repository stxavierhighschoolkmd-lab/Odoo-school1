...
describe("drop invisible elements", () => {
    ...
    describe("invisible snippets have an indicator in snippet dialog telling they are invisible", () => {
        test("mobile invisible snippets", async () => {
            await setupWebsiteBuilder(`<section>test</section>`, {
            ...
            expect(
                ".o_add_snippet_dialog :iframe .o_custom_snippet_edit .o_visibility_indicator_mobile_invisible"
            ).toHaveCount(1);
        });

        test("desktop invisible snippets", async () => {
            await setupWebsiteBuilder(`<section>test</section>`, {
                snippets: getSnippetInfos(snippetDesktopInvisible),
            });
            await contains(
            ...
            await waitFor(".o_add_snippet_dialog :iframe .o_custom_snippet_edit");
            expect(
                ".o_add_snippet_dialog :iframe .o_custom_snippet_edit .o_visibility_indicator_desktop_invisible"
            ).toHaveCount(1);
        });

        test("conditionally visible snippets", async () => {
            await setupWebsiteBuilder(`<section>test</section>`, {
                snippets: getSnippetInfos(snippetConditionalInvisible),
            });
            await contains(
            ...
            await waitFor(".o_add_snippet_dialog :iframe .o_custom_snippet_edit");
            expect(
                ".o_add_snippet_dialog :iframe .o_custom_snippet_edit .o_visibility_indicator_conditional"
            ).toHaveCount(1);
        });
    });
});
