...
class TestSampleGenerator(TransactionCase):
    ...
    def test_sample_generator_basic(self):
        values = ['a', 'b', 'c']
        generator = Sample(field=self.name_field, env=self.env, values=values, null_frac=0)

        values_generated = [next(generator) for _ in range(20)]
    ...
    def test_sample_generator_weights(self):
        ...
        values_generated = [next(generator) for _ in range(50)]
