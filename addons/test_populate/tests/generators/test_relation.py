...
class TestRelationOneDynamicDomain(TransactionCase):
    ...
    def test_dynamic_domain_filters_per_call(self):
        ...
        for _ in range(10):
            val_us = generator._next({'country_code': 'US'})
            self.assertIn(val_us, us_suppliers.ids)

            val_ca = generator._next({'country_code': 'CA'})
    ...
    def test_get_kwargs_static_domain(self):
        attrs = {'domain': "[('country_code', '=', 'US')]"}
        kwargs = RelationOne.get_kwargs(attrs)
        self.assertIsInstance(kwargs['domain'], Domain)

    def test_get_kwargs_dynamic_domain(self):
