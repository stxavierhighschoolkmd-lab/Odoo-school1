...
class TestBatchStockETransport(TestETransportFlows):
    ...
    def test_multistep_delivery_validation_constraints(self):
        ...
        sale_order = self.env['sale.order'].create({
            ...
            'order_line': [
                Command.create({'product_id': self.product_a.id, 'product_uom_qty': 1.0}),
            ],
    ...
    def test_batch_validation_with_missing_carrier(self):
        ...
        self.assertRecordValues(pickings.sorted('sale_id'), [
            {'sale_id': sale_orders[0].id, 'l10n_ro_edi_stock_enable': True, 'l10n_ro_edi_stock_enable_send': False, 'picking_type_code': 'outgoing'},
            {'sale_id': sale_orders[1].id, 'l10n_ro_edi_stock_enable': True, 'l10n_ro_edi_stock_enable_send': False, 'picking_type_code': 'outgoing'},
        ])
    ...
    def test_etransport_send_button_visibility_logic(self):
        ...
        self.assertRecordValues(pickings.sorted('sale_id'), [
            ...
            {'sale_id': sale_orders[0].id, 'l10n_ro_edi_stock_enable': True, 'l10n_ro_edi_stock_enable_send': True, 'picking_type_code': 'outgoing'},
            # In the batch so should be send in the batch
            {'sale_id': sale_orders[1].id, 'l10n_ro_edi_stock_enable': True, 'l10n_ro_edi_stock_enable_send': False, 'picking_type_code': 'outgoing'},
        ])
        self.assertRecordValues(batch, [
