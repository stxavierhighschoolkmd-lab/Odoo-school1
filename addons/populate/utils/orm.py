...
class DynamicDomain:
    ...
    **Example**:
    ...
    _resolver: Callable[..., DomainType]

    def __new__(cls, domain_expr: str) -> DynamicDomain | Domain:
        dynamic_fields = list(get_expression_field_names(domain_expr))
        if not dynamic_fields:
            return Domain(literal_eval(domain_expr))

        self = super().__new__(cls)
        object.__setattr__(self, 'dynamic_fields', dynamic_fields)
        object.__setattr__(self, '_resolver', safe_eval(f'lambda {', '.join(dynamic_fields)}: {domain_expr}'))
        return self

    def __call__(self, **depends: ValuesType) -> DomainType:
...
def get_ref_domain(
    ...
    """Given a ref for a specific model, return a domain representing does records."""
