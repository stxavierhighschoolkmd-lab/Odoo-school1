...
class HttpError403 extends Interaction {
    ...
    appendDemoErrorCard() {
        ...
        if (this.errorBlockEl) {
            this.originalContentEl = this.errorBlockEl.firstElementChild;
            this.demoErrorCard = renderToElement("website.http_error_403_demo_template");
            this.errorBlockEl.replaceChild(this.demoErrorCard, this.originalContentEl);
        }
    }

    destroy() {
        if (this.errorBlockEl) {
            this.errorBlockEl.replaceChild(this.originalContentEl, this.demoErrorCard);
        }
    }
}
