...
class Distribution(ABC):
    ...
    def __str__(self):
        params = ', '.join(f"{k}={v}" for k, v in sorted(self.__dict__.items()) if k != 'rng')
        if params:
            return f"{self.__class__.__name__}({params})"
        return f"{self.__class__.__name__}"

    def __repr__(self):
        params = ', '.join(f"{k}={v!r}" for k, v in sorted(self.__dict__.items()) if k != 'rng')
        if params:
...
class WeightedDistribution(Distribution):
    ...
    def choices[T](self, seq: Sequence[T], *, k: int = 1) -> list[T]:
        # ignore the arg, used baked-in values
        return self.rng.choices(self.values, weights=self.weights, k=k)
...
class UniformDistribution(Distribution):
    ...
    def sample(self):
        return self.rng.uniform(self.min, self.max) if self.is_bounded else self.rng.random()
