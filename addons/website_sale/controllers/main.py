...
class WebsiteSale(payment_portal.PaymentPortal):
    ...
    def shop_address_submit(
        ...
        **form_data,
        ...
        if redirect == SHOP_PATH:
            return request.make_json_response(redirect_dict)  # Cart was reset during validation
    ...
    def _create_new_address(
        ...
        return (
            request.env["res.partner"].sudo().with_context(creation_context).create(address_values)
        )
    ...
    def _include_country_and_state_in_address(self, address):
        ...
        country = request.env["res.country"].search(
            [("code", "=", address.pop("country"))], limit=1
        )
        state_id = False
