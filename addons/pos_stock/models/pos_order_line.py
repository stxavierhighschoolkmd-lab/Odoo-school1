...
class PosOrderLine(models.Model):
    ...
    def _prepare_refund_data(self, refund_order, lot=False):
        refund_data = super()._prepare_refund_data(refund_order, lot)
        refund_data['pack_lot_ids'] = lot
        return refund_data

    def _get_product_cost(self, stock_moves=False):
        self.ensure_one()
        ...
        if moves and self._is_product_storable_fifo_avco():
            ...
            if cost_currency.is_zero(product_cost) and self.order_id.shipping_date:
                if self.refunded_orderline_id:
                    return self.refunded_orderline_id.total_cost / self.refunded_orderline_id.qty
                return product.standard_price
            return product_cost
        return super()._get_product_cost(stock_moves)
