...
class AccountEdiCommon(models.AbstractModel):
    ...
    def _import_lines(self, record, tree, xpath, document_type=False, tax_type=False, qty_factor=1):
        ...
        for line_tree in tree.iterfind(xpath):
            ...
            lines_values += self._retrieve_line_allowance_charges(record, line_values, line_values['tax_ids'])
    ...
    @api.model
    def _retrieve_rebate_val(self, record, tree, xpath_dict, quantity):
    ...
    @api.model
    def _retrieve_allowance_charge_vals(self, record, tree, xpath_dict, quantity):
        ...
        def _is_allowance_charge_node_valid():
            """
        ...
        for allowance_charge_node in tree.iterfind(xpath_dict['allowance_charge']):
            ...
            if _is_allowance_charge_node_valid():
                ...
                if (
                    ...
                    and not self._retrieve_allowance_charge_tax(record.company_id, vals)
                ):
    ...
    def _retrieve_line_vals(self, record, tree, document_type=False, qty_factor=1):
        ...
        # rebate (optional)
        rebate = self._retrieve_rebate_val(record, tree, xpath_dict, quantity)

        # Charges are collected (they are used to create new lines), Allowances are transformed into discounts
        discount_amount, allowance_charge_vals, allowance_charge_logs = self._retrieve_allowance_charge_vals(record, tree, xpath_dict, quantity)
        logs += allowance_charge_logs
    ...
    def _retrieve_allowance_charge_tax(self, company_id, allowance_charge_tax_vals):
        ...
        - If reason_code is provided:
        1. not price_include: reason_code + reason (if available), then reason_code only
        2. price_include: reason_code + reason (if available), then reason_code only

        - If reason_code is not provided:
        3. not price_include: reason only
        4. price_include: reason only
        """
        ...
        amount = (
            ...
            else (allowance_charge_tax_vals['amount'] / qty)
        )
        ...
        base_domain = [
            *self.env['account.journal']._check_company_domain(company_id),
            ...
            ('amount', '=', amount * (1 if is_charge else -1)),
        ...
        for domain in domains:
            for price_include in (False, True):
                tax_domain = domain + [('price_include', '=', price_include)]
    ...
    def _retrieve_line_allowance_charges(self, record, line_values, taxes):
        """
        ...
        Otherwise, a new aml is created to represent the allowance/charge.
        """
        line_vals = []
        percent_taxes = self.env['account.tax'].search([
            ('id', 'in', taxes),
            ('amount_type', '=', 'percent')
        ]).ids
        tax_map = [
        ...
        # Otherwise create separate lines to avoid wrong tax base interactions.
        if all(tax for _, tax in tax_map):
            ...
            for val, _ in tax_map:
                sign = 1 if val['charge_indicator'] == 'true' else -1
                line_vals.append([
                    val['reason_code'] + " " + val['reason'],
                    1,
                    sign * val['amount'],
                    percent_taxes,
                ])
        return record._get_line_vals_list(line_vals)
