...
export class TableBorderPlugin extends Plugin {
        ...
        toolbar_items: [
            withSequence(10, {
                ...
                props: {
                    ...
                    colorPrefix: "--",
                    getSelectedColors: () => {
                        this.selectedBorderColors.color = this.getTableSelectedBorder("color");
     ...
     */
    getCellBorder(cell, subProperty) {
     ...
     * @returns {string} property value
     */
    getTableSelectedBorder(subProperty) {
        ...
        for (const cell of table.querySelectorAll(".o_selected_td")) {
            const cellValue = this.getCellBorder(cell, subProperty);
        ...
        if (value === "") {
            for (const table of tables) {
                for (const prop of ["color", "width", "style"]) {
                    for (const cell of table.querySelectorAll(".o_selected_td")) {
                        cell.style.removeProperty(`border-${prop}`);
                    }
                }
            }
            return;
