...
class TestSlidesManagement(slides_common.SlidesCase):
    ...
    def test_merge_partners_with_unique_courses(self):
        ...
        course1, course2 = self.env['slide.channel'].create([{'name': 'Course 1'}, {'name': 'Course 2'}])
    ...
    def test_merge_partners_with_duplicate_courses(self):
        """ Test that merging partners with common courses fails """
        course1, course2 = self.env['slide.channel'].create([{'name': 'Course 1'}, {'name': 'Course 2'}])
        partner1, partner2 = self.env['res.partner'].create([
            {'name': 'Partner 1', 'email': 'partner1@example.com'},
            {'name': 'Partner 2', 'email': 'partner2@example.com'}])
        course1.sudo()._action_add_members(partner1 | partner2)
        course2.sudo()._action_add_members(partner1 | partner2)
        wizard = self.env['base.partner.merge.automatic.wizard'].create({})
        ...
        self.assertEqual(
            ...
            "You cannot merge these contacts because they are enrolled in the following common courses: Course 1, Course 2"
        )
