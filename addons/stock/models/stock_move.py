...
Please change the quantity done or the rounding precision in your settings.""",
    ...
    def _prepare_procurement_qty(self):
        consumed_from_stock_dict = self.env.context.get('consumed_from_stock_dict')
        quantities = []
        ...
        for move in self:
            ...
            reserved_qty = min(move.product_qty, free_qty)
            consumed_from_stock_dict[move.location_id, move.product_id.id] += reserved_qty
