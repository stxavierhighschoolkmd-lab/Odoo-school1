...
class Job(models.Model):
    ...
    @api.private
    def execute(self, generators: Mapping[str, Generator] | None = None, *, seed: int | None = None):
        """
        Execute a job.
        ...
        assert self.seed is not False or seed is not None
        ...
        with self.execution_scope():
            ...
            if generators is None:
                generators = self._create_generators(seed)
            ...
            if self.is_executable:
                ...
                for subjob in self.pending_subjobs:
                    subjob.execute(generators, seed=seed)
    ...
    def _create_generators(self, seed: int | None = None) -> Mapping[str, Generator]:
        self.ensure_one()
        generators = {}
        model = self.env[self.model_name]
        valid_fields = self.instructions.keys()
        rng = Random(seed or self.seed)
