...
class Binary(Generator):
    ...
    """
    name = 'binary.binary'
    allowed_fields_type = ['binary', 'virtual']
    ...
    def _next(self, known_vals: ValuesType) -> BinaryBytes | bool:
        return BinaryBytes(self.rng.randbytes(self.size))

    @classmethod
    def get_kwargs(cls, attrs: dict) -> dict:
...
class Image(Generator):
    ...
    """
    name = 'binary.image'
    allowed_fields_type = ['binary', 'virtual']  # `image` fields have type='binary' in the ORM
