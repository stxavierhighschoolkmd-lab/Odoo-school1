...
class L10n_Es_Edi_TbaiDocument(models.Model):
        ...
        return {
            ...
            'headers': {
                ...
                'eus-bizkaia-n3-data': json.dumps({
                    ...
                    'inte': {
                        'nif': self.env['res.partner']._extract_vat_without_country_code(company.vat, 'ES'),
                        'nrs': company.name,
                    },
                    'drs': {
    ...
    def _generate_final_xml_bi(self, freelancer=False):
        ...
        lroe_values = {
            ...
            'sender': sender,
            'sender_vat': self.env['res.partner']._extract_vat_without_country_code(sender.vat, 'ES'),
    ...
    def _get_recipient_values(self, partner, is_simplified=False):
        ...
        if not partner._l10n_es_is_foreign() and partner.vat:
            recipient_values['nif'] = partner._extract_vat_without_country_code(partner.vat, 'ES')
    ...
    def _get_tbai_id(self):
        ...
        tbai_id_no_crc = '-'.join([
            'TBAI',
            str(self.env['res.partner']._extract_vat_without_country_code(company.vat, 'ES')),
