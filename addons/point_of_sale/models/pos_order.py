...
class PosOrder(models.Model):
    ...
    def _generate_order_invoice(self):
        ...
        if (self.to_invoice or has_paylater_pm) and self.state == 'paid' and self.config_id.journal_id:
    ...
    def _compute_is_invoiced(self):
        for order in self:
            is_invoiced = bool(order.account_move)
            session_sales_move = order.session_id.sales_move_id
            session_refunds_move = order.session_id.refunds_move_id

            if order.is_refund:
                is_invoiced = is_invoiced and order.account_move != session_refunds_move
            else:
                is_invoiced = is_invoiced and order.account_move != session_sales_move

            order.is_invoiced = is_invoiced
            order.invoice_status = 'invoiced' if is_invoiced else 'to_invoice'
    ...
    def _refund(self):
        ...
        for order in self:
            ...
            for line in order.lines.filtered(lambda line: line.refunded_qty < line.qty):
    ...
    def _aggregate_base_line_and_prepare_account_move_line_data(self, base_lines):
        ...
        for base_line in aggregated:
            ...
            name = product.name if product else _("PoS Miscellaneous")
            to_create.append({
                'name': name,
                'quantity': 1.0,
                'price_unit': tax_details['total_excluded_currency'],
        ...
        for idx, line in enumerate(self.lines):
            ...
            if line.product_id.type == 'combo':
                to_create.append({
                    ...
                    'name': f"{line.product_id.name} x {line.qty}",
                    'quantity': line.qty,
            ...
            if is_percentage and price_changed < 0:
                ...
                name = _(
                    'Price discount from %s to %s',
                    float_repr(line.product_id.lst_price, decimal),
                    float_repr(line.price_unit, decimal),
                )
                to_create.append({
                    'name': name,
                    'display_type': 'line_note',
    ...
    def _prepare_account_move_line_data(self, aggregate=True):
        ...
        for order in self:
            base_lines = order.lines._prepare_base_lines_for_taxes_computation()
            ...
            if order.is_refund:
                refund_base_lines.extend(base_lines)
            else:
                sale_base_lines.extend(base_lines)
        ...
        if not aggregate:
            ...
            sale_lines = self._prepare_account_move_line_data_from_base_line(sale_base_lines)
            refund_lines = self._prepare_account_move_line_data_from_base_line(refund_base_lines)
            return sale_lines, refund_lines

        # 2. Aggregate product lines by (revenue account + VAT rate) grouping
        aggregated_per_line = self._aggregate_base_line_and_prepare_account_move_line_data(sale_base_lines)
        aggregated_per_refund = self._aggregate_base_line_and_prepare_account_move_line_data(refund_base_lines)
        return aggregated_per_line, aggregated_per_refund

    def _prepare_out_receipt_payment_line_data(self, partner=None):
        """
        ...
        for payment in self.payment_ids:
            ...
            # Invoiced orders are handled by the individual-invoice flow
            # all split_transactions pm will be excluded from the combined flow
    ...
    @api.model
    def _create_payment_moves(self, session, payments, partner=None):
        """
        Create one record per PM slot AFTER posting:
        - cash PMs  => account.bank.statement.line (counterpart on POS receivable)
        - other PMs => account.payment
        Each produces a credit line on the PM receivable that reconciles against
        the receipt's debit payment_term line.

        !!! Only give the partner arguments if you want to use its receivable
        account instead of the session's default one (e.g. for invoiced orders)
        """
        # Customer account is handled elsewhere...
        all_payment_lines = self.env['account.move.line']
        for pm_data in payments:
            pm = pm_data['metadata']['payment_method_id']
            amount = pm_data['account.move.line']['amount_currency']

            if pm.type == 'cash':
                all_payment_lines |= pm._create_cash_payment_line(session, amount, partner)
            elif pm.type == 'bank':
                all_payment_lines |= pm._create_bank_payment_line(session, amount, partner)
    ...
    def action_pos_order_invoice(self):
        ...
        if not self.is_invoiced:
            if self._should_create_picking_real_time() and self.session_id.state != 'closed':
                self._create_order_picking()
    ...
    def _prepare_invoice_vals(self):
        ...
        move_type = 'out_refund' if self.is_refund else 'out_invoice'
        ...
        lines = self._prepare_account_move_line_data(False)
        line_commands = [Command.create(line) for line in lines[0]]  # sales lines
        line_commands += [Command.create(line) for line in lines[1]]  # refund
        ...
        if refunded_order_invoice:
            vals['ref'] = _('Reversal of: %s', refunded_order_invoice.name)
            vals['reversed_entry_id'] = refunded_order_invoice.id

        check_rounding = config.cash_rounding and config.only_round_cash_method
        is_cash_payments = any(p.payment_method_id.is_cash_count for p in self.payment_ids)
        if check_rounding or (config.cash_rounding and is_cash_payments):
            vals['invoice_cash_rounding_id'] = config.rounding_method.id
    ...
    def _generate_pos_order_invoice(self):
        ...
        payments = self._prepare_out_receipt_payment_line_data(self.partner_id)

        # Build payment_term line Commands, one per PM slot.
        # display_type='payment_term' + asset_receivable account satisfies
        # the _check_payable_receivable constraint on sale documents.
        line_data = [pm['account.move.line'] for pm in payments]
        vals['line_ids'] = [Command.create(pm_data) for pm_data in line_data]
        ...
        all_payment_lines = self._create_payment_moves(
            self.session_id,
            payments,
            self.partner_id,
        )
        ...
        if self.config_id.cash_rounding and invoice.invoice_cash_rounding_id:
            ...
            difference_curr = sign * (amount_paid - invoice.amount_total)
            difference_balance = currency.round(difference_curr / rate) if rate else 0.0

            if not currency.is_zero(difference_curr):
                ...
                if rounding_line:
                    line_ids_commands.append(Command.update(rounding_line.id, {
                        'amount_currency': rounding_line.amount_currency + difference_curr,
                        'balance': rounding_line.balance + difference_balance,
                    }))
                else:
                    if difference_curr > 0.0:
                    ...
                    line_ids_commands.append(Command.create({
                        ...
                        'amount_currency': difference_curr,
                ...
                line_ids_commands.append(Command.update(existing_terms_line.id, {
                    'amount_currency': existing_terms_line.amount_currency - difference_curr,
    ...
    def _generate_invoice_after_session_closing(self):
        ...
        global_move = (
            session.refunds_move_id if self.is_refund else session.sales_move_id
        )
        ...
        ref = _(
            "Reversal of order %s from session %s", self.name, session.name,
        )
        reversal_move = self.env["account.move"].create({
            "move_type": "out_refund"
            if global_move.move_type == "out_invoice"
            else "out_invoice",
            "journal_id": global_move.journal_id.id,
        ...
        for line in self.lines:
            base_lines = line._prepare_base_lines_for_taxes_computation()
            ...
            for base_line in base_lines:
                ...
                product_lines.append(Command.create({
                    "name": name,
                    "quantity": 1.0,
                    "price_unit": sign * tax_details["total_excluded_currency"],
                    "display_type": "product",
                    "account_id": base_line["_grouping_key"]["account_id"].id,
                    "tax_ids": [(6, 0, base_line["tax_ids"].ids)],
                    "tax_tag_ids": [(6, 0, tax_details["tax_tag_ids"].ids)],
                    "pos_order_line_id": line.id,
                }))

                for tax_rep_line in tax_details.get("tax_details_per_rep", []):
                    tax_lines.append(Command.create({
                        "name": tax_rep_line["tax_rep_line_id"].tax_id.name,
                        "account_id": tax_rep_line["account_id"].id,
                        "amount_currency": sign
                        * tax_rep_line["amount_currency"],
                        "balance": sign * tax_rep_line["balance"],
                        "tax_line_id": tax_rep_line["tax_rep_line_id"].id,
                        "tax_tag_ids": [
                            (6, 0, tax_rep_line["tax_tag_ids"].ids),
                        ],
                        "display_type": "tax",
                    }))
        ...
        reversal_move.with_context(skip_invoice_sync=True)._post()
        self.reversed_move_ids = reversal_move

        payment_term_lines = global_move.line_ids.filtered(
            lambda line: line.display_type == "payment_term",
        )
        ...
        if payment_term_lines and reversal_payment_lines:
            to_reconcile = payment_term_lines | reversal_payment_lines
            to_reconcile.with_context(skip_invoice_sync=True).reconcile()
