...
class PropertyDefinition(Generator):
    ...
    allowed_fields_type = ['properties_definition']
    ...
    def get_kwargs(cls, attrs):
        ...
        if 'allowed_types' in attrs:
            kwargs['allowed_types'] = const_eval(attrs['allowed_types'])

        if 'possible_values' in attrs:
            kwargs['possible_values'] = const_eval(attrs['possible_values'])

        if 'entries' in attrs:
            kwargs['entries'] = const_eval(attrs['entries'])
...
class PropertyValue(Generator):
    ...
    allowed_fields_type = ['properties']
    ...
    def __init__(self, field: fields.Properties, **kwargs):
        ...
        super().__init__(field=field, depends=[field.definition_record], null_frac=0, **kwargs)
