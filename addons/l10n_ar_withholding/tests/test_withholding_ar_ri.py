...
class TestArWithholdingArRi(TestArCommon):
    ...
    def test_12_withholding_check_payment_iterative_flush(self):
        ...
        """
        self.tax_wth_test_1.write({'amount': 1})
        ...
        if own_check_method.id not in third_party_check_journal.outbound_payment_method_line_ids.mapped('payment_method_id').ids:
        ...
        invoice = self.env['account.move'].create({
            "ref": "Vendor Bill Adhoc test 30250",
            "partner_id": self.res_partner_adhoc.id,
            "move_type": "in_invoice",
            "invoice_line_ids": [(0, 0, {
