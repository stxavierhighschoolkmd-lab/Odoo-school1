...
describe("collaborative makeSavePoint", () => {
    test("After a savePoint, local commits should be discarded in collaboration and external commits should not", async () => {
        const peerInfos = await setupMultiEditor({
            peerIds: ["c1", "c2"],
...
describe("history addExternalCommit", () => {
    test("should revert and re-apply local mutations that are not part of a finished commit", async () => {
        ...
        // was not adjusted properly when receiving the external commit
    ...
    test("should be able to retreive commits when disconnected from peers that has send commit", async () => {
        await testMultiEditor({
            ...
            afterCreate: (peerInfos) => {
                ...
                peerInfos.c2.collaborationPlugin.onExternalHistoryCommits([
                    peerInfos.c1.historyPlugin.commits[1],
                ]);
                insert(peerInfos.c2.editor, "e");
                peerInfos.c1.collaborationPlugin.onExternalHistoryCommits([
                    peerInfos.c2.historyPlugin.commits[2],
                ]);
                peerInfos.c3.collaborationPlugin.onExternalHistoryCommits([
                    peerInfos.c2.historyPlugin.commits[2],
                ]);
...
describe("selection", () => {
    test("should rectify a selection offset after an external commit", async () => {
        const peerInfos = await setupMultiEditor({
            peerIds: ["c1", "c2"],
            contentBefore: `<p>a[c1}{c1][c2}{c2]</p>`,
        });
...
describe("Collaboration with embedded components", () => {
    ...
    test("components are mounted and destroyed during addExternalCommit", async () => {
        let index = 1;
    ...
    describe("Embedded state", () => {
        ...
        test("New component with an embedded state received from a collaborator can have its state when it hasn't finished being mounted", async () => {
            ...
            // but it was not correctly updated after external commits. To update
